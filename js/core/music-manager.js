  ...
  async loadTracksFromAPI() {
    try {
      ...
      if (cachedTracks && cachedTracks.length > 0) {
        ...
        console.log('¹ï¸ No cached tracks found, fetching from API...');
      }
      ...
      if (this.availableTracks.length === 0) {
        ...
        console.log('¹ï¸ Falling back to demo tracks...');
        this.availableTracks = this.getDemoTracks();
        ...
        if (maxEnergyLevel < 5) {
          ...
          console.log(` Energy filter applied: ${beforeFilter} â†’ ${this.availableTracks.length} tracks (max energy: ${maxEnergyLevel})`);
        }
      ...
      console.error('Stack trace:', error.stack);
      console.log('¹ï¸ Attempting to use demo tracks as fallback...');
      this.availableTracks = this.getDemoTracks();
